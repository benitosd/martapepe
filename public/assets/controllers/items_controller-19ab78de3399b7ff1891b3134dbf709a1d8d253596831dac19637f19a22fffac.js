...
export default class extends Controller {
  ...
  getDatasetParams(datasetKeys) {
    const params = {};
    datasetKeys.forEach((key) => {
      params[key] = this.data.get(key) || "";
    });
    return params;
  }
  ...
  async updateField(event, endpoint, datasetKeys, targetMapping) {
    ...
    const params = this.getDatasetParams(datasetKeys);
    params[datasetKeys[0]] = currentFieldId; // Actualiza el campo actual
    this.data.set(datasetKeys[0], currentFieldId);
